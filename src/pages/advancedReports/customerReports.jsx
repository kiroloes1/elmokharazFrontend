...
// قم بتعديل مسار الـ API حسب إعدادات Axios في مشروعك
...
export default function CustomersReport() {
  ...
  // حالة كشف الحساب التفصيلي (العميل المحدد)
  ...
  const fetchCustomersReport = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "حدث خطأ أثناء جلب تقرير العملاء"
      );
    } finally {
      setLoading(false);
    }
  ...
  // 2. جلب كشف الحساب التفصيلي لعميل محدد
    ...
    <div className="p-4 md:p-6 bg-ligth min-h-screen text-dark space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-brown/20 pb-4">
        <div>
          ...
          <p className="text-xs text-brown mt-1">
            متابعة المديونيات الحالية، حركات النقل، وكشوفات الحساب التفصيلية لكل عميل.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-brown/10 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 text-xs">
          {/* البحث باسم العميل */}
          <div>
            <label className="block text-brown font-medium mb-1">اسم العميل</label>
            <input
              type="text"
              name="name"
              value={filters.name}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-brown/10 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-brown/70 text-sm">
            لا يوجد عملاء مطبقين لخيارات البحث الحالية.
          </div>
        ) : (
      ...
      {/* Modal / Drawer: كشف حساب العميل التفصيلي */}
      {selectedCustomer && (
        <div className="fixed inset-0 z-50 bg-dark/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl border border-brown/20 w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
            {/* Modal Header */}
            <div className="p-4 bg-ligth border-b border-brown/10 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4 text-xs text-brown mt-0.5">
            ...
            {/* Modal Filters */}
            <div className="p-3 bg-white border-b border-brown/10 flex flex-wrap items-center gap-3 text-xs">
              <div className="flex items-center gap-2">
            ...
            <div className="p-4 overflow-y-auto flex-1">
              {txLoading ? (
                ...
                <div className="p-8 text-center text-brown/70 text-xs">
                  لا توجد نقلات مسجلة لهذا العميل في الفترة المحددة.
                </div>
              ) : (
                      ...
                      <tbody className="divide-y divide-brown/10">
                        {transactionsData.map((tx) => (
                          <tr key={tx._id} className="hover:bg-ligth/30">
                            <td className="p-2.5 text-brown font-medium">
                              {tx.deliveryDate
                            ...
                            <td className="p-2.5 text-brown/80 max-w-xs truncate">
                              {tx.notes || "—"}
                            </td>
                          </tr>
                        ))}
