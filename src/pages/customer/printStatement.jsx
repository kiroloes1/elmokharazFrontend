        ...
        <div className="mb-8 border-r-4 border-black pr-4 space-y-1">
          ...
          {customerData?.openningBalance !== undefined && customerData?.openningBalance !== 0 && (
            <p className="text-md text-slate-800 font-semibold">
              الرصيد الافتتاحي: <span className="font-bold">{formatAmount(customerData.openningBalance)}</span>
            </p>
          )}
                      ...
                      {(isDelivery || isDebt || isOpening || isPay) ? (
                        <div className="space-y-1 text-right">
                          <div className="text-[15px] font-semibold text-black">
                            ...
                            {!isOpening && formatAmount(op.previousBalance)}
                          </div>
                          <div className="text-[15px] font-semibold text-black">
                            {isDelivery && (
                              <><b>المديونية الحالية: </b>{formatAmount(op.currentInvoiceAmount)}</>
                            )}
                            {isDebt && (
                            ...
                            {isOpening && (
                              <><b>المديونية السابقة: </b>{formatAmount(op.currentInvoiceAmount)}</>
                            )}
                          </div>
                              ...
                              <>
                                {op.deliveryPayments.map((p, idx) => (
                                  <span key={`pay-${idx}`} className="text-[13px] text-slate-600 block">
                                    {translatePaymentMethod(p.paymentMethod)}: {formatAmount(p.amount)}
                                  </span>
                                ))}
                              </>
                            )}
                              ...
                              <>
                                {op.deliveryPaymentMethods.map((p, idx) => (
                                  <span key={`del-${idx}`} className="text-[13px] text-slate-600 block">
                                    {translatePaymentMethod(p.paymentMethod)}: {formatAmount(p.paidAmount)}
                                  </span>
                                ))}
                              </>
                            )}
