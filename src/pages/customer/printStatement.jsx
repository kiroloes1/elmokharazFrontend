...
const DeliveryStatement = () => {
  ...
  // ✅ تجميع الحركات (المديونية السابقة + الـ Payments الغير delivery + الـ Delivery المجمعة)
 ...
 deliveries.forEach((deliveryDetail) => {
  ...
  if (tea > 0) {
    paymentReceived += tea;
        ...
        // pay: التاجر دفع للمصنع → يزيد الرصيد (+)
        debtAdded = op.amount || 0;
        paymentReceived = 0;
        currentInvoiceAmount = op.amount;
      } 
      else if ( isPay ) {
        // debt: المصنع سداد للتاجر → يقل الرصيد (-)
        debtAdded = 0;
...
<div className="mb-8 border-r-4 border-black pr-4 space-y-1">
  <p className="text-2xl font-bold text-gray-900">اسم التاجر: {customerData?.name || "---"}</p>
  <p className="text-md text-slate-800 font-semibold">رقم التلفون: {customerData?.phone || "---"}</p>
    ...
    <p className="text-md text-slate-800">
      <span className="font-semibold">
        {customerData.openningBalance < 0 
          ? "رصيد سابق لصالحنا (فلوس لينا): " 
          : "مديونية سابقة (فلوس علينا): "}
      </span>
      <span className="font-bold">{Math.abs(customerData.openningBalance).toLocaleString()} ج.م</span>
    </p>
  )}
  ...
  {(() => {
    ...
    const isOwedToUs = currentBalance < 0; // فلوس لينا
    const isOwedToSupplier = currentBalance > 0; // فلوس علينا
    ...
    return (
      <p className="text-md text-slate-800">
        ...
        <span className="text-xs text-gray-600 mr-2 font-bold">
          {isOwedToUs && "(فلوس لينا)"}
          {isOwedToSupplier && "(فلوس علينا)"}
          {!isOwedToUs && !isOwedToSupplier && "(الحساب متخلص)"}
        </span>
      </p>
    );
  })()}
...
{   !isOpening &&                 <td className="border border-black p-2 text-center font-normal  text-black leading-relaxed">
...
منذ البدايه
                    ...
                    <td className="border border-black p-2">
                      ...
                      {isDelivery && op.details?.items && (
                        <div className="mt-1 w-full">
                          ...
                          <div className="mt-2 flex flex-col bg-gray-100/80 p-2 border border-gray-300 rounded-sm font-normal  text-[15px] gap-1.5">
                            ...
                            {isDelivery && op.tea > 0 && (
                              <div className="text-slate-900 border-t border-dashed border-gray-300 pt-1.5 flex flex-col gap-1 text-[13px]">
                                ...
                                <div className="flex justify-between font-normal  text-[15px] bg-white p-1.5 rounded border border-gray-200">
                      ...
                      {isPay && op.paymentDetails && (
                        <div className="mt-2 text-sm">
                          <div className="font-normal ">طريقة الدفع: {translatePaymentMethod(op.paymentMethod)}</div>
                      ...
                      {isOpening  && (
                        <div className="mt-2 text-sm text-700 font-normal ">
                          {op.note || "رصيد أول المدة"} - {currOpen >0 ?" يضاف إلى رصيد التاجر" :" يخصم من حساب التاجر"}
                        </div>
                      )}
                    </td>
                        ...
                        <div className="space-y-1 text-right">
                         ...
                         {  !isOpening && <b>{  "المديونية السابقة"}:</b> }
                          ...
                          <div className="text-[15px] font-semibold text-black">
                            ...
                            {isDebt && (
                              <><b>سداد من التاجر(استلام فلوس):</b> +{op.debtAdded?.toLocaleString()} ج.م</>
                            )}
                            { isPay && (
                          ...
                          <div className="text-[15px] font-normal  text-slate-800 border-t border-dashed pt-1 mt-1">
                    ...
                    <td className="border border-black p-2 text-center font-semibold text-slate-700 text-md bg-slate-50/5 pt-3">
                      {isDelivery  && (
                        <div className="flex flex-col items-center gap-1">
                          {/* <span className="font-normal  text-md">{op.paymentReceived.toLocaleString()} ج.م</span> */}
                          <span className="font-normal  text-md">{op.deliveryPayments.reduce((acc,curr)=>acc+ curr.amount,0)}</span>
                      ...
                      { isDebt &&(
                        <div className="flex flex-col items-center gap-0.5">
                          <span className="font-normal  text-md text-700">+{op.amount.toLocaleString()} ج.م</span>
                          <span className="text-[13px] font-normal  block">(سداد من التاجر(استلام فلوس))</span>
                          {op.paymentMethod && (
  ...
  {(() => {
  const finalBalance = sequentialLedger.length > 0 
    ? (sequentialLedger[sequentialLedger.length - 1]?.balance || 0)
    : (customerData?.openningBalance || 0);

  const isOwedToUs = finalBalance < 0; // فلوس لينا (مطلوب من التاجر)
  const isOwedToSupplier = finalBalance > 0; // فلوس علينا (مستحق للتاجر)

  return (
    <div className="flex justify-between items-center text-xl pt-2 border-t-2 border-black">
      <span className="">
        {isOwedToUs && "المبلغ المطلوب من التاجر (فلوس لينا):"}
        {isOwedToSupplier && "الرصيد النهائي المستحق للتاجر (فلوس علينا):"}
        {!isOwedToUs && !isOwedToSupplier && "الرصيد النهائي:"}
      </span>
