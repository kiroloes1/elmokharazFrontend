...
const SupplierList = () => {
  ...
  const updatePaymentHistory = async () => {
    try {
      ...
      await api.patch(
        `/suppliers/editPaymentHistory/${editData.id}/${selectedSupplier._id}`,
        payload
      );
  ...
  const deletePaymentHistory = async (paymentId) => {
    ...
    try {
      await api.delete(
        `/suppliers/deletePaymentHistory/${paymentId}/${selectedSupplier._id}`
      );
  ...
  const renderModuleBadge = (module, moneyFlow) => {
    ...
    const badges = {
      pay: { label:"دفع للتاجر", color: "bg-emerald-50 text-emerald-600" },
      equipment: { label: "شراء معدات ", color: "bg-blue-50 text-blue-600" },
      debt: { label: "زياده مديونية", color: "bg-red-50 text-red-600" },
      wire: { label: " سلك", color: "bg-blue-50 text-blue-600" },
      maintenance: { label: "صيانه ", color: "bg-blue-50 text-blue-600" },
      bag: { label: "شكاير ", color: "bg-blue-50 text-blue-600" },
      equipment_supply: { label: "صيانه معدات", color: "bg-blue-50 text-blue-600" },
      ...
      <div className="md:w-1/3 w-full flex flex-col bg-white rounded-[24px]  border border-slate-100 ">
        <div className="p-6 border-b border-slate-50">
          <h2 className="text-xl font-black text-dark mb-4 flex items-center gap-2">
            <User className="text-dark -500" /> قائمة التاجرين
          </h2>
            ...
            <div className="bg-white p-6 md:p-8 rounded-[24px] border border-slate-100 relative ">
              ...
              <div className="relative flex flex-col md:flex-row justify-between items-start gap-6">
                ...
                <div className="flex flex-wrap gap-2 w-full md:w-auto justify-end">
                  <button
                    onClick={() => navigate(`/supplier/printSupplierDetails/${selectedSupplier?._id}`)}
                 ...
                 <div className="flex  gap-10 md:gap-2">
                  <button
                    onClick={() => navigate(`/supplier/edit/${selectedSupplier._id}`)}
                          ...
                          <td className="p-5">
                            <div className="flex items-center gap-3">
                              {t.moduleId && (
                                <Eye
                                  size={20}
                                  onClick={() => navigate(`/equipment/print/${t.moduleId}`)}
                                  className="text-dark -600 cursor-pointer hover:text-dark -500"
                                  title="عرض المستند المتعلق"
                                />
                              )}
                              {(t.module =="pay" || t.module =="debt" )  &&  !(t.walletInfo?.linkWallet || false) &&(
                                <Edit
                                  size={20}
