  ...
  return (
    <div id='invoice' className={`min-h-screen  p-0 md:p-6 text-right font-${settings?.invoiceFont}}  text-black`} dir="rtl" style={{fontFamily:`${settings?.invoiceFont} ||cairo `}}>
      ...
      >
              ...
              <tr>
                <td className="align-top">
                  ...
                  <div className="text-xs font-bold text-black mt-3 space-y-1">
                    <p className="m-0">السيد التاجر: <span className="font-black">{supplier?.name || delivery.supplier?.name || "تاجر عام"}</span></p>
                    <p className="m-0">رصيد التاجر : <span className="font-black">{ delivery.supplier?.balance || 0}</span></p>
                ...
                <td className="text-left align-top min-w-[160px]">
                  <div className="p-3 border-2 border-black inline-block text-right">
                    <p className="font-black text-xs text-black m-0">نقلة رقم: #{delivery.delveryNumber}</p>
        ...
        <div className="mb-4">
          <h3 className="text-xs font-black text-black mb-2 border-r-4 border-black pr-2 uppercase">تفاصيل أصناف النقلة الحالية</h3>
    ...
    .map((item, idx) => {
      ...
      return (
        <tr key={item._id || idx} className="text-black">
          ...
          <td className="p-2 text-left border-r border-black font-black">
          ...
          <div className="grid grid-cols-4 gap-2 my-3 text-xs">
            <div className="border border-black p-2 text-center">
              ...
              <div className="font-black text-black text-sm mt-0.5">{totalWeight.toLocaleString()} كجم</div>
            </div>
            <div className="border border-black p-2 text-center">
              <div className="font-bold text-black text-[10px]">إجمالي الراجع الحالي</div>
              <div className="font-black text-black text-sm mt-0.5">{returnWeight.toLocaleString()} كجم</div>
            </div>
            <div className="border border-black p-2 text-center">
              <div className="font-bold text-black text-[10px]">صافي الوزن الحالي</div>
              <div className="font-black text-black text-sm mt-0.5">{totalNetWeight.toLocaleString()} كجم</div>
            </div>
            <div className="border-2 border-black p-2 text-center bg-gray-50">
              ...
              <div className="font-black text-black text-sm mt-0.5">{currentNetItemsTotal.toLocaleString()} ج.م</div>
            </div>
          </div>
        </div>
        ...
        {oldReturnItems.length > 0 && (
          <div className="mb-4">
            <h3 className="text-xs font-black text-black mb-2 border-r-4 border-black pr-2 uppercase">تفاصيل الراجع القديم (مخصوم)</h3>
            <table className="w-full border-collapse border-2 border-black text-xs text-center">
              ...
              <tbody className="divide-y divide-black">
                {oldReturnItems.map((item) => (
                  <tr key={item._id}>
                    <td className="border-r border-black p-2 text-right font-black">{item.item?.name}</td>
                    ...
                    <td className="p-2 text-left font-black">{(item.oldReturnWeight * item.pricePerKg).toLocaleString()} ج.م</td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="font-black border-t-2 border-black bg-gray-50">
                <tr>
                  ...
                  <td colSpan={2} className="p-2 text-left">
                    إجمالي المخصوم: <span className="font-black">{totalOldReturnValue.toLocaleString()} ج.م</span>
                  </td>
                </tr>
              </tfoot>
            </table>
                  ...
                  <div className="border-2 border-black overflow-hidden">
                      ...
                      <table className="w-full text-right">
                        <tbody className="-">
                          ...
                          <tr>
                            ...
                            <td className="font-black text-black text-left py-1.5">-{currentNetItemsTotal.toLocaleString()} ج.م</td>
                          </tr>
                          <tr className='border-t-2'>
                            <td className="text-black font-bold py-1.5">المجموع (القديم + النقلة):</td>
                            <td className="font-black text-black text-left py-1.5">{(oldBalance - currentNetItemsTotal).toLocaleString()} ج.م</td>
                          </tr>
                          {totalOldReturnValue > 0 && (
                            <tr>
                              <td className="text-black font-bold py-1.5">اجمالي  راجع قديم:</td>
                              <td className="font-black text-black text-left py-1.5">+{totalOldReturnValue.toLocaleString()} ج.م</td>
                            </tr>
                          )}
                          {teaForWorkers > 0 && (
                            <tr>
                              <td className="text-black font-bold py-1.5">إكرامية/شاي عمال:</td>
                              <td className="font-black text-black text-left py-1.5">-{teaForWorkers.toLocaleString()} ج.م</td>
                            </tr>
                          )}
                          <tr>
                            <td className="text-black font-bold py-1.5">إجمالي المدفوع الآن:</td>
                            <td className="font-black text-black text-left py-1.5">+{paidAmount.toLocaleString()} ج.م</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                    ...
                    <div className="bg-gray-100 p-3 text-black border-t-2 border-black">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm font-black">{((oldBalance - currentNetItemsTotal)+totalOldReturnValue +paidAmount).toLocaleString()} ج.م</span>
                      </div>
                    </div>
                  </div>
                </td>
                ...
                <td className="align-top w-1/2 pr-2">
                  <div className="space-y-3">
                    <div className="border border-black p-3 bg-white">
                      <h4 className="text-[11px] font-black text-black mb-2 border-b border-black pb-1 m-0">تفاصيل وسائط الدفع للنقلة:</h4>
