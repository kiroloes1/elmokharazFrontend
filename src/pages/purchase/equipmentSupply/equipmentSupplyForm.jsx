...
const EquipmentSupply = () => {
  ...
  // قائمة مستلزمات المعدات (EqupimnetPart) للاقتراح التلقائي
  ...
  const [formData, setFormData] = useState({
    ...
    // اسم المعدة (حقل مطلوب في الموديل EquipmentSupply) - يتم اختياره بالبحث من قائمة EqupimnetPart
    equipmentName: "",
    notes: "",
    note: "", // بيان حركة الخزنة العام (يُستخدم في الدفعات)
    payment: [
      {
        paidAmount: 0,
        paymentMethod: "cash",
  ...
  // ===== جلب اقتراحات المحافظ لربط عمليات الدفع =====
  ...
  const addItem = () => {
    setFormData({
      ...
      items: [
        ...formData.items,
        { itemName: "", type: "", quantity: 1, unitPrice: 0, notes: "" },
      ],
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post("/equipmentSupply", payload);
      showAlert({ title: "تم تسجيل فاتورة شراء مستلزمات المعدات وتحديث الحسابات بنجاح", icon: "success" });
      ...
      <form onSubmit={handleSubmit} className="space-y-6 h-[100vh] overflow-auto pb-32 pr-2">
        ...
        <div className="space-y-4">
          <textarea
            ...
            placeholder="ملاحظات عامة توثيقية إضافية تُحفظ بملف فاتورة المعدات..."
            rows="2"
          ...
          >
            {loading ? (
              "جاري ترحيل القيود وحفظ البيانات..."
            ) : (
              <>
                <Save size={22} /> إنشاء فاتورة مستلزمات المعدات وربط الحسابات المالية
              </>
            )}
          </button>
        </div>
