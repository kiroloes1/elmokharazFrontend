...
const PrintEquipmentSupplyPage = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const invoiceRes = await api.get(`/equipmentSupply/${id}`);
  ...
  const handleSharePDF = async () => {
    ...
    const arabicFileName = `فاتورة_مستلزمات_معدات_${invoice.invoiceNumber}.pdf`;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          ...
          text: `مرفق فاتورة مشتريات مستلزمات معدات من: ${supplier?.name || invoice.supplier?.name || "تاجر  عام"}`
        });
      } else {
...
export default PrintEquipmentSupplyPage;