...
const EquipmentSupplyEdit = () => {
  ...
  // قائمة مستلزمات المعدات (EqupimnetPart) للاقتراح التلقائي على اسم المعدة
  const [equipmentPartsList, setEquipmentPartsList] = useState([]);
  ...
  // ===== جلب بيانات الفاتورة والتجار =====
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [supRes, invoiceRes] = await Promise.all([
          api.get("/suppliers"),
          api.get(`/equipmentSupply/${id}`),
        ]);

        const sups = supRes.data.data || supRes.data.suppliers || [];
        setSuppliers(sups);

        const rawData = invoiceRes.data.equipment || invoiceRes.data.purchaseEquipment || invoiceRes.data.data || invoiceRes.data;
        ...
        if (rawData) {
          ...
          // تحويل الأصناف - مطابق لموديل EquipmentSupply (itemName)
          ...
          setFormData({
            ...
            equipmentName: rawData.equipmentName || "",
            notes: rawData.notes || "",
            note: rawData.note || "",
            items: formattedItems,
            payment: formattedPayments,
            invoiceNumber: rawData.invoiceNumber || null,
          });
          ...
          if (currentSupplierName) {
            ...
            const foundSup = sups.find((s) => s._id === (rawData.supplier?._id || rawData.supplier));
            if (foundSup) setSupSearch(foundSup.name);
          }
        }
      } catch (err) {
        console.error("Error fetching data", err);
        showAlert({ title: "خطأ في جلب بيانات فاتورة مستلزمات المعدات", icon: "error" });
      } finally {
  ...
  // ===== جلب مستلزمات المعدات (EqupimnetPart) للاقتراح =====
  const fetchEquipmentParts = async () => {
    try {
      const response = await api.get("/equipmnetpart");
      if (response.data.success) {
        setEquipmentPartsList(response.data.data);
      }
    } catch (err) {
      showAlert({
        title: "حدث خطأ أثناء جلب بيانات مستلزمات المعدات",
        icon: "error",
      });
    }
  };
  ...
  // ===== حسابات الأصناف =====
  const calculateItemTotal = (item) => Number(item.quantity || 0) * Number(item.unitPrice || 0);
  ...
  const addItem = () => {
    setFormData({
      ...
      items: [
        ...formData.items,
        { itemName: "", type: "", quantity: 1, unitPrice: 0, total: 0, notes: "" },
      ],
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      formData.items.some(
        (i) => !i.itemName || Number(i.quantity) <= 0 || Number(i.unitPrice) < 0
      )
    )
      return showAlert({ title: "تأكد من بيانات المستلزمات (الاسم، الكمية، السعر)", icon: "warning" });
    ...
    try {
      ...
      // عدّل هذا المسار ليطابق راوت فاتورة مستلزمات المعدات الفعلي عندك
      await api.put(`/equipmentSupply/${id}`, payload);
      showAlert({ title: "تم تعديل فاتورة مستلزمات المعدات بنجاح", icon: "success" });
      navigate("/equipmentSupply");
    } catch (err) {
  ...
  if (fetchLoading) {
    return (
      <div className="w-full h-screen flex items-center justify-center bg-ligth/10">
        <div className="text-dark font-black text-lg">جاري تحميل بيانات فاتورة مستلزمات المعدات...</div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full mx-auto p-4 lg:p-8 bg-ligth/10" dir="rtl">
      <form onSubmit={handleSubmit} className="space-y-6 h-[100vh] overflow-auto pb-32 pr-2">
        {/* قسم التاجر واسم المعدة والتاريخ */}
        <div className="bg-white p-6 rounded-2xl border border-brwonLight shadow-sm grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
          ...
          <div className="space-y-2 relative text-right" ref={equipmentRef}>
            ...
            {showEquipmentList && equipmentSearch.length > 0 && (
              <div className="absolute z-50 w-full mt-2 bg-white border border-brwonLight rounded-xl shadow-2xl max-h-60 overflow-y-auto">
                {equipmentPartsList
                  .filter((eq) => eq.itemName.toLowerCase().includes(equipmentSearch.toLowerCase()))
                  .slice(0, 10)
                  .map((eq) => (
                    <div
                      key={eq._id}
                      ...
                      onClick={() => {
                        setFormData({ ...formData, equipmentName: eq.itemName });
        ...
        <div className="space-y-4">
          <div className="sticky top-0 z-30 bg-ligth/10 backdrop-blur py-2 flex justify-between items-center border-b border-brown/10 mb-4">
            <h3 className="text-lg font-black text-dark flex items-center gap-2">
              <Wrench size={22} className="text-brown" /> مستلزمات فاتورة المعدات
            </h3>
            <button
              type="button"
              onClick={addItem}
            ...
            >
              <Plus size={18} /> إضافة مستلزم جديد
            </button>
          </div>
                ...
                >
                  <div className="space-y-3">
                    <div className="text-right">
                      <label className="text-xs font-black text-dark">اسم المستلزم</label>
                      <input
                        type="text"
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="text-right">
                        ...
                        <input
                          type="number"
                          min="1"
                          className="w-full p-2.5 mt-1 bg-ligth/20 border border-brown/10 rounded-xl font-black text-dark outline-none focus:border-brown"
                          value={item.quantity === 0 ? "" : item.quantity}
                          onChange={(e) =>
                            handleItemChange(idx, "quantity", Math.max(0, Number(e.target.value)))
                          }
                        />
                      </div>
                      <div className="text-right">
                    ...
                    <div className="text-right">
                      <label className="text-xs font-black text-dark">ملاحظات على المستلزم</label>
                      <input
                        type="text"
                        className="w-full p-2.5 mt-1 bg-ligth/20 border border-brown/10 rounded-xl outline-none font-bold text-dark focus:border-brown"
                        placeholder="اختياري"
                  ...
                  <div className="pt-2 border-t border-brown/5">
                    <div className="bg-ligth/20 p-3 rounded-xl flex justify-between items-center">
                      <span className="text-xs font-black text-dark/80">إجمالي المستلزم</span>
                      <span className="font-black text-sm text-brown">{total.toLocaleString()} EGP</span>
                    </div>
                  </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-brwonLight shadow-sm space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-1 gap-6">
            ...
            {/* بيان حركة الخزنة وملاحظات الفاتورة */}
          ...
          <div className="bg-white border border-brwonLight rounded-2xl overflow-hidden shadow-lg">
            <div className="bg-ligth p-4 flex justify-between items-center border-b border-brwonLight">
              <h4 className="font-black text-dark flex items-center gap-2 text-sm">
                <Notebook size={18} className="text-brown" /> ملخص وتأثير الحساب المالي (تعديل الفاتورة #{formData.invoiceNumber})
              </h4>
            ...
            <div className="p-5 space-y-3.5 font-bold text-xs text-dark">
              <div className="flex justify-between items-center text-dark/80">
                <span>إجمالي قيمة فاتورة المعدات:</span>
              ...
              <div className="bg-ligth/30 p-3 rounded-xl border border-brown/5 flex justify-between items-center">
                <span className="text-dark/80 font-black">المتبقي من هذه الفاتورة:</span>
                <span className="text-base font-black text-brown">{remainingAmount.toLocaleString()} EGP</span>
              </div>
              ...
              <div className="flex justify-between items-center text-dark/80 border-t border-brwonLight/30 pt-2">
                ...
                >
                  {remainingAmount <= 0 ? "مدفوع بالكامل" : totalPaidAmount > 0 ? "مدفوع جزئياً" : "غير مدفوع"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end pt-4">
            ...
            >
              <Save size={20} />
              {loading ? "جاري التحديث..." : "تحديث فاتورة مستلزمات المعدات"}
            </button>
          </div>
        </div>
      </form>
...
export default EquipmentSupplyEdit;
