...
const MaintenanceEdit = () => {
  ...
  // قائمة المعدات للاقتراح التلقائي
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const rawData = invoiceRes.data.maintain
 || invoiceRes.data.purchaseEquipment || invoiceRes.data.data || invoiceRes.data;
        ...
        if (rawData) {
          ...
          if (rawData.returnDate) {
            formattedReturnDate = rawData.returnDate.split("T")[0];
          }

          // تحويل الأصناف - مطابق لموديل Maintenance (partName, faultDescription, repairCost)
          const formattedItems = (rawData.items || []).map((i) => ({
            partName: i.partName || "",
            faultDescription: i.faultDescription || "",
            repairCost: i.repairCost || 0,
            notes: i.notes || "",
          }));
          ...
          setFormData({
            ...
            maintenanceProvider: rawData.maintenanceProvider || "",
            notes: rawData.notes || "",
            note: rawData.note || "",
        ...
        showAlert({ title: "خطأ في جلب بيانات فاتورة الصيانة", icon: "error" });
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showAlert({ title: "تم تعديل فاتورة الصيانة بنجاح", icon: "success" });
  ...
  if (fetchLoading) {
    return (
      <div className="w-full h-screen flex items-center justify-center bg-ligth/10">
        <div className="text-dark font-black text-lg">جاري تحميل بيانات فاتورة الصيانة...</div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full mx-auto p-4 lg:p-8 bg-ligth/10" dir="rtl">
      <form onSubmit={handleSubmit} className="space-y-6 h-[100vh] overflow-auto pb-32 pr-2">
        {/* قسم التاجر واسم المعدة وجهة الصيانة والتاريخ */}
        ...
        <div className="bg-white p-6 rounded-2xl border border-brwonLight shadow-sm space-y-6">
          ...
          <div className="bg-white border border-brwonLight rounded-2xl overflow-hidden shadow-lg">
            <div className="bg-ligth p-4 flex justify-between items-center border-b border-brwonLight">
              <h4 className="font-black text-dark flex items-center gap-2 text-sm">
                <Notebook size={18} className="text-brown" /> ملخص وتأثير الحساب المالي (تعديل فاتورة الصيانة #{formData.invoiceNumber})
              </h4>
          ...
          <div className="flex justify-end pt-4">
            ...
            >
              <Save size={20} />
              {loading ? "جاري التحديث..." : "تحديث فاتورة الصيانة"}
            </button>
          </div>
        </div>
      </form>
