...
import { Plus, Trash2, Save, Package, Search, Notebook, CreditCard, Calendar } from "lucide-react";
...
const EditBagForm = () => {
  ...
  // قائمة أنواع الشكاير للاقتراح التلقائي
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const rawData = invoiceRes.data.bag || invoiceRes.data.data || invoiceRes.data;
        ...
        if (rawData) {
          ...
          // تحويل الأصناف - مطابق لموديل BagPurchase
          const formattedItems = (rawData.items || []).map((i) => ({
            bagType: i.bagType?._id || i.bagType || "",
            bagTypeName: i.bagType?.name || "",
            size: i.size || "",
        ...
        showAlert({ title: "خطأ في جلب بيانات فاتورة شراء الشكاير", icon: "error" });
      } finally {
  ...
  // ===== جلب أنواع الشكاير للاقتراح =====
  const fetchBagTypes = async () => {
    try {
      const response = await api.get("/bagtype");
      if (response.data.bagTypes) {
        const bagNames = response.data.bagTypes.map(bt => ({
          _id: bt._id,
          name: bt.name,
          notes: bt.notes
        }));
        setBagTypes(bagNames);
      }
    } catch (err) {
      showAlert({
        title: "حدث خطأ أثناء جلب بيانات أنواع الشكاير",
        icon: "error",
      });
    }
  };
  ...
  const addItem = () => {
    setFormData({
      ...
      items: [
        ...formData.items,
        { bagType: "", bagTypeName: "", size: "", quantity: 1, unitPrice: 0, total: 0, notes: "" },
      ],
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      formData.items.some(
        (i) => !i.bagType || !i.size || Number(i.quantity) < 1 || Number(i.unitPrice) < 0
      )
    ) {
      return showAlert({ 
        title: "تأكد من بيانات الشكاير (نوع الشكارة، المقاس، الكمية، السعر)", 
        icon: "warning" 
      });
    }
    ...
    try {
      ...
      await api.put(`/bag/${id}`, payload);
      showAlert({ title: "تم تعديل فاتورة شراء الشكاير بنجاح", icon: "success" });
      navigate("/bag");
    } catch (err) {
  ...
  if (fetchLoading) {
    return (
      <div className="w-full h-screen flex items-center justify-center bg-ligth/10">
        <div className="text-dark font-black text-lg">جاري تحميل بيانات فاتورة شراء الشكاير...</div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full mx-auto p-4 lg:p-8 bg-ligth/10" dir="rtl">
      <form onSubmit={handleSubmit} className="space-y-6 h-[100vh] overflow-auto pb-32 pr-2">
        {/* قسم التاجر والتاريخ ورقم الفاتورة */}
        ...
        <div className="space-y-4">
          <div className="sticky top-0 z-30 bg-ligth/10 backdrop-blur py-2 flex justify-between items-center border-b border-brown/10 mb-4">
            <h3 className="text-lg font-black text-dark flex items-center gap-2">
              <Package size={22} className="text-brown" /> أصناف الشكاير المشتراة
            </h3>
            <button
              type="button"
              onClick={addItem}
            ...
            >
              <Plus size={18} /> إضافة شكارة جديدة
            </button>
          </div>
              ...
              return (
                ...
                >
                  <div className="space-y-3">
                    {/* نوع الشكارة مع اقتراح تلقائي */}
                    <div className="text-right relative" ref={bagRef}>
                      <label className="text-xs font-black text-dark">نوع الشكارة</label>
                      <div className="relative">
                        ...
                        <input
                          ...
                          placeholder="ابحث عن نوع الشكارة..."
                          value={item.bagTypeName || ""}
                          onFocus={() => setShowBagList(true)}
                          onChange={(e) => {
                            const value = e.target.value;
                            setShowBagList(true);
                            const found = bagTypes.find(w => w.name === value);
                            if (found) {
                              handleItemChange(idx, "bagType", found._id);
                              handleItemChange(idx, "bagTypeName", found.name);
                            } else {
                              handleItemChange(idx, "bagType", "");
                              handleItemChange(idx, "bagTypeName", value);
                            }
                          }}
                        />
                      </div>
                      {showBagList && bagTypes.length > 0 && (
                        <div className="absolute z-50 w-full mt-1 bg-white border border-brwonLight rounded-xl shadow-2xl max-h-40 overflow-y-auto">
                          {bagTypes
                            .filter((bt) => 
                              bt.name.toLowerCase().includes((item.bagTypeName || "").toLowerCase())
                            )
                            .map((bt) => (
                              <div
                                key={bt._id}
                                ...
                                onClick={() => {
                                  handleItemChange(idx, "bagType", bt._id);
                                  handleItemChange(idx, "bagTypeName", bt.name);
                                  setShowBagList(false);
                                }}
                              >
                                {bt.name}
        ...
        <div className="bg-white p-6 rounded-2xl border border-brwonLight shadow-sm space-y-6">
          ...
          <div className="bg-white border border-brwonLight rounded-2xl overflow-hidden shadow-lg">
            <div className="bg-ligth p-4 flex justify-between items-center border-b border-brwonLight">
              <h4 className="font-black text-dark flex items-center gap-2 text-sm">
                <Notebook size={18} className="text-brown" /> ملخص وتأثير الحساب المالي (تعديل فاتورة شراء الشكاير #{formData.invoiceNumber})
              </h4>
            ...
            <div className="p-5 space-y-3.5 font-bold text-xs text-dark">
              <div className="flex justify-between items-center text-dark/80">
                <span>إجمالي قيمة الشكاير المشتراة:</span>
          ...
          <div className="flex justify-end pt-4">
            ...
            >
              <Save size={20} />
              {loading ? "جاري التحديث..." : "تحديث فاتورة شراء الشكاير"}
            </button>
          </div>
        </div>
      </form>
