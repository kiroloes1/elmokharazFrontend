...
const CreateBagForm = () => {
  ...
  // قائمة أنواع الشكاير (Bag Types) للاقتراح التلقائي
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post("/bag", payload);
      showAlert({ title: "تم تسجيل فاتورة شراء الشكاير وتحديث الحسابات بنجاح", icon: "success" });
      navigate("/bag");
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6 h-[100vh] overflow-auto pb-32 pr-2">
        ...
        <div className="space-y-4">
          <textarea
            ...
            placeholder="ملاحظات عامة توثيقية إضافية تُحفظ بملف فاتورة شراء الشكاير..."
            rows="2"
          ...
          >
            {loading ? (
              "جاري ترحيل القيود وحفظ بيانات شراء الشكاير..."
            ) : (
              <>
                <Save size={22} /> إنشاء فاتورة شراء شكاير وربط الحسابات المالية
              </>
            )}
          </button>
        </div>
