...
const BagTypeManager = () => {
  const [bagTypeList, setBagTypeList] = useState([]);
  ...
  // 1. جلب أنواع الشنط من الـ Backend
  const fetchBagTypes = async () => {
    try {
      setLoading(true);
      const response = await api.get("/bagtype");
      // الـ Backend يرجع { count: X, bagTypes: [...] }
      setBagTypeList(response.data.bagTypes || []);
    } catch (err) {
      showAlert({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim()) {
      showAlert({
        title: "يرجى كتابة اسم نوع الشنطة",
        icon: "warning",
      });
      return;
    }
    ...
    try {
      if (modalMode === "create") {
        const response = await api.post("/bagtype", { name, notes });
        showAlert({
          title: response.data.message || "تم إضافة نوع الشنطة بنجاح",
          icon: "success",
        });
      } else {
        const response = await api.put(`/bagtype/${selectedId}`, {
          name,
          notes,
        });
        showAlert({
          title: response.data.message || "تم تعديل نوع الشنطة بنجاح",
          icon: "success",
        });
      }
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await api.delete(`/bagtype/${id}`);
        showAlert({
          title: response.data.message || "تم حذف نوع الشنطة بنجاح",
          icon: "success",
        });
        fetchBagTypes();
      } catch (err) {
  ...
  return (
    <div className="min-h-screen text-dark p-4 md:p-8 font-sans" dir="rtl">
      <div className="mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-lg shadow-sm border border-brown/10">
          <div>
            <h1 className="text-2xl font-bold text-dark">إدارة أنواع الشنط</h1>
            <p className="text-dark/60 text-sm mt-1">عرض، إضافة وتعديل أنواع الشنط في النظام</p>
          </div>
          <button
        ...
        <div className="bg-white rounded-lg shadow-sm border border-brown/10 overflow-hidden">
          ...
          <div className="overflow-x-auto">
            ...
            ) : bagTypeList.length === 0 ? (
              <div className="p-12 text-center text-dark/50">
                لا توجد أنواع شنط مضافة حالياً.
              </div>
            ) : (
              <table className="w-full text-right border-collapse">
                <thead>
                  <tr className="bg-ligth/80 border-b border-brown/10 text-dark/70 text-sm">
                    ...
                    <th className="p-4 font-semibold">اسم نوع الشنطة</th>
          ...
          <div className="bg-white rounded-lg shadow-xl border border-brown/10 w-full max-w-md overflow-hidden transform transition-all">
            ...
            <div className="p-5 bg-dark text-white flex justify-between items-center">
              <h3 className="font-bold text-lg">
                {modalMode === "create" ? "إضافة نوع شنطة جديد" : "تعديل نوع الشنطة"}
              </h3>
              <button
                onClick={closeModal}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-dark mb-2">
                  اسم نوع الشنطة <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  placeholder="أدخل اسم نوع الشنطة..."
                  value={name}
