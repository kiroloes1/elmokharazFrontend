...
import { 
  Package, Calendar, CreditCard, Edit3, Loader2, 
  Box, Printer, Trash, ChevronRight, ChevronLeft,
  DollarSign
} from "lucide-react";
...
const BagList = () => {
  ...
  const fetchInvoiceDetails = useCallback(async (id) => {
    try {
      ...
      setSelectedInvoice(res.data.bag || null);
    } catch (err) {
  ...
  const fetchInvoices = useCallback(async (currentPage) => {
    try {
      setLoading(true);
      const res = await api.get(`/bag?page=${currentPage}&limit=${limit}`);
      const rawInvoices = res.data.bags || [];
  ...
  if (!loading && invoices.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-ligth text-center p-6" dir="rtl">
        <Package size={70} className="text-brown/40 mb-4" />
        <h2 className="text-2xl font-black text-dark">لا توجد فواتير شراء شكاير</h2>
        <p className="text-dark/60 mt-2">لم يتم تسجيل أي فاتورة شراء شكاير حتى الآن.</p>
        <button
          onClick={() => navigate("/bag/add")}
          className="mt-6 bg-brown text-ligth px-6 py-3 rounded-lg font-black hover:opacity-90 transition-all"
        >
          إضافة فاتورة شراء شكاير جديدة
        </button>
      </div>
    );
  }
        ...
        <div className="flex justify-between items-center border-b border-brown/20 pb-4 flex-wrap gap-4">
          <div className="space-y-1 text-right">
            ...
            <h1 className="text-3xl font-black text-dark">
              فاتورة شراء شكاير #{selectedInvoice?.invoiceNumber || "---"} - {selectedInvoice?.supplier?.name || "تاجر  غير محدد"}
            </h1>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              onClick={() => navigate(`/bag/edit/${selectedInvoice?._id}`)}
              disabled={!selectedInvoice || detailsLoading}
            ...
            <button
              onClick={() => navigate(`/bag/print/${selectedInvoice?._id}`)}
              disabled={!selectedInvoice || detailsLoading}
            ...
            >
              <Printer size={18} /> طباعة
                        ...
                        <button
              onClick={() => navigate(`/bag/part/print/${selectedInvoice?._id}`)}
              disabled={!selectedInvoice || detailsLoading}
              className="bg-brown hover:opacity-90 disabled:opacity-50 text-ligth px-5 py-2.5 rounded-lg font-black text-sm flex items-center gap-2 shadow-md transition-all"
            >
              <Printer size={18} /> طباعة الشكاير
            </button>
          </div>
        </div>
              ...
              <div className="lg:col-span-7 bg-white p-6 rounded-md border border-brown/20 shadow-sm text-right space-y-4">
                <div className="flex justify-between items-center border-b border-brown/10 pb-3">
                  <h3 className="font-black text-dark text-lg flex items-center gap-2">
                    <Package size={20} className="text-accent" /> أصناف الشكاير المشتراة
                  </h3>
                  ...
                  <table className="w-full text-right border-collapse text-xs">
                    <thead>
                      <tr className="bg-ligth text-dark border-b border-brown/20">
                        ...
                        <th className="p-3 font-black">نوع الشكارة</th>
                      ...
                      {selectedInvoice.items?.map((item, idx) => {
                        ...
                        return (
                          <tr key={idx} className="hover:bg-ligth/50 font-bold text-dark">
                            ...
                            <td className="p-3 font-black">{item.bagType?.name || "بدون نوع"}</td>
                ...
                {selectedInvoice.notes && (
                  <div className="bg-brown/10 p-3 rounded-lg border border-brown/20 mt-4">
      ...
      <div className="border-t border-brown/20 pt-4 space-y-3">
        ...
        <div className="flex items-center gap-3 overflow-x-auto max-w-[96vw] lg:max-w-[81vw] pb-3 pt-1 scrollbar-thin">
          {invoices.map((inv) => {
            ...
            const totalQty = getTotalQuantity(inv.items);
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between items-center text-xs">
                  <span className={`${isSelected ? "text-ligth/60" : "text-dark/50"}`}>
                    أصناف: {inv.items?.length || 0}
                  </span>
                  <span className={`${isSelected ? "text-ligth/60" : "text-dark/50"}`}>
                    كمية: {totalQty}
                  </span>
