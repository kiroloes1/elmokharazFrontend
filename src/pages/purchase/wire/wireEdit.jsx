...
const EditWireForm = () => {
  ...
  // قائمة أنواع الأسلاك للاقتراح التلقائي
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const rawData = invoiceRes.data.wire || invoiceRes.data.data || invoiceRes.data;
        ...
        if (rawData) {
          ...
          // تحويل الأصناف - مطابق لموديل WirePurchase
          const formattedItems = (rawData.items || []).map((i) => ({
            wireType: i.wireType?._id || i.wireType || "",
            wireTypeName: i.wireType?.name || "",
            size: i.size || "",
        ...
        showAlert({ title: "خطأ في جلب بيانات فاتورة شراء الأسلاك", icon: "error" });
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.put(`/wire/${id}`, payload);
      showAlert({ title: "تم تعديل فاتورة شراء الأسلاك بنجاح", icon: "success" });
      navigate("/wire");
    } catch (err) {
  ...
  if (fetchLoading) {
    return (
      <div className="w-full h-screen flex items-center justify-center bg-ligth/10">
        <div className="text-dark font-black text-lg">جاري تحميل بيانات فاتورة شراء الأسلاك...</div>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-6 h-[100vh] overflow-auto pb-32 pr-2">
        ...
        <div className="bg-white p-6 rounded-2xl border border-brwonLight shadow-sm space-y-6">
          ...
          <div className="bg-white border border-brwonLight rounded-2xl overflow-hidden shadow-lg">
            <div className="bg-ligth p-4 flex justify-between items-center border-b border-brwonLight">
              <h4 className="font-black text-dark flex items-center gap-2 text-sm">
                <Notebook size={18} className="text-brown" /> ملخص وتأثير الحساب المالي (تعديل فاتورة شراء الأسلاك #{formData.invoiceNumber})
              </h4>
          ...
          <div className="flex justify-end pt-4">
            ...
            >
              <Save size={20} />
              {loading ? "جاري التحديث..." : "تحديث فاتورة شراء الأسلاك"}
            </button>
          </div>
        </div>
      </form>
