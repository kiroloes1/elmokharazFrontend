...
const WireList = () => {
  ...
  const fetchInvoiceDetails = useCallback(async (id) => {
    try {
      ...
      setSelectedInvoice(res.data.wire || null);
    } catch (err) {
  ...
  const fetchInvoices = useCallback(async (currentPage) => {
    try {
      setLoading(true);
      const res = await api.get(`/wire?page=${currentPage}&limit=${limit}`);
      const rawInvoices = res.data.wires || [];
