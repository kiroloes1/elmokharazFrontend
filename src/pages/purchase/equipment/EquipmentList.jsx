...
const EquipmentList = () => {
  ...
  const fetchInvoiceDetails = useCallback(async (id) => {
    try {
      ...
      const res = await api.get(`/equipmnet/${id}`);
  ...
  const fetchInvoices = useCallback(async (currentPage) => {
    try {
      setLoading(true);
      const res = await api.get(`/equipmnet?page=${currentPage}&limit=${limit}`);
  ...
  const handleToDelete = async (id) => {
    try {
      const confirm = await showAlertConfirm({
        title: "حذف فاتورة مشتريات معدات",
  ...
  if (!loading && invoices.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-ligth text-center p-6" dir="rtl">
        ...
        <h2 className="text-2xl font-black text-dark">لا توجد فواتير مشتريات معدات</h2>
  ...
  const currentTotalPaid = currentPayments.reduce((acc, p) => acc + Number(p.amount || 0), 0);
  const totalAmount = selectedInvoice?.totalAmount || 0;
  const remainingAmount = selectedInvoice?.remainingAmount || (totalAmount - currentTotalPaid);
      ...
      <div className="flex-1 mb-6 space-y-6">
        ...
        <div className="flex justify-between items-center border-b border-brown/20 pb-4 flex-wrap gap-4">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              onClick={() => navigate(`/equipment/edit/${selectedInvoice?._id}`)}
              disabled={!selectedInvoice || detailsLoading}
            ...
            <button
              onClick={() => navigate(`/equipment/print/${selectedInvoice?._id}`)}
              disabled={!selectedInvoice || detailsLoading}
          ...
          <button
              onClick={() => navigate(`/equipment/part/print/${selectedInvoice?._id}`)}
              disabled={!selectedInvoice || detailsLoading}
              className="bg-brown hover:opacity-90 disabled:opacity-50 text-ligth px-5 py-2.5 rounded-lg font-black text-sm flex items-center gap-2 shadow-md transition-all"
            >
              <Printer size={18} /> طباعة المعدات
            </button>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-5 rounded-md border border-brown/20 shadow-sm grid grid-cols-2 md:grid-cols-3 gap-4 text-right">
              ...
              <div className="flex items-center justify-between bg-slate-50 p-3 rounded-lg">
                ...
                <span className="font-black text-dark text-sm">{selectedInvoice.receivedBy?.username || "---"}</span>
              </div>
              <div className="flex items-center justify-between bg-slate-50 p-3 rounded-lg">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              {/* === الجانب الأيمن: جدول المعدات (7 أعمدة) === */}
              <div className="lg:col-span-7 bg-white p-6 rounded-md border border-brown/20 shadow-sm text-right space-y-4">
                <div className="flex justify-between items-center border-b border-brown/10 pb-3">
                  <h3 className="font-black text-dark text-lg flex items-center gap-2">
                    <Wrench size={20} className="text-accent" /> المعدات والكميات
                  </h3>
                  ...
                  <table className="w-full text-right border-collapse text-xs">
                    <thead>
                      <tr className="bg-ligth text-dark border-b border-brown/20">
                        <th className="p-3 font-black rounded-r-xl">اسم المعدة</th>
                      ...
                      {selectedInvoice.items?.map((item, idx) => {
                        ...
                        return (
                          <tr key={idx} className="hover:bg-ligth/50 font-bold text-dark">
                            <td className="p-3 font-black">{item.equipmentName || "بدون اسم"}</td>
                            <td className="p-3 text-center">{item.type || item.category || "—"}</td>
                ...
                <div className="grid grid-cols-1 md:grid-cols-1 gap-4 mt-4">
                  {(selectedInvoice.oldBalance >=0 || selectedInvoice.oldBalance <=0) && (
              ...
              <div className="lg:col-span-5 bg-white p-6 rounded-md border border-brown/20 shadow-sm text-right space-y-4">
                ...
                <div className="space-y-3 max-h-[380px] overflow-y-auto pr-1">
                  {currentPayments.length > 0 ? (
                        ...
                        <div key={idx} className="p-3.5 rounded-lg border border-brown/20 space-y-2 text-xs">
                          ...
                          {p.walletInfo && (
                            <div className="text-[11px] text-dark/70 space-y-0.5 pt-0.5">
                              ...
                              {p.walletInfo.provider && <p><span className="text-dark/40">المزود:</span> {p.walletInfo.provider}</p>}
                            </div>
                          )}
                          {p.cheque && (
                            <div className="text-[11px] text-dark/70 space-y-0.5 pt-0.5">
                              <p><span className="text-dark/40">رقم الشيك:</span> {p.cheque.chequeNumber}</p>
                              <p><span className="text-dark/40">البنك:</span> {p.cheque.bankName}</p>
                              <p>
                    ...
                    <div className="text-center py-8 text-dark/40 text-sm">
