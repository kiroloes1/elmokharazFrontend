...
const [activeEquipmentIdx, setActiveEquipmentIdx] = useState(null);
  ...
  useEffect(() => {
    const fetchSuppliers = async () => {
      try {
        // عدّل هذا المسار ليطابق راوت التاجرين الفعلي عندك (app.use('/v1/suppliers', suppliers))
  ...
  const addItem = () => {
    setFormData({
      ...
      items: [
        ...formData.items,
        { equipmentName: "", type: "", quantity: 1, unitPrice: 0, notes: "" },
      ],
    });
  };
  ...
  // ===== إدارة طرق الدفع (نفس منطق نموذج النقلة) =====
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      formData.items.some(
        (i) => !i.equipmentName || Number(i.quantity) <= 0 || Number(i.unitPrice) < 0
      )
    )
      return showAlert({ title: "تأكد من بيانات المعدات (الاسم، الكمية، السعر)", icon: "warning" });
    ...
    try {
      ...
      // عدّل هذا المسار ليطابق راوت المعدات الفعلي عندك (app.use('/v1/equipmnet', equipmnet))
      await api.post("/equipmnet", payload);
      showAlert({ title: "تم تسجيل فاتورة شراء المعدات وتحديث الحسابات بنجاح", icon: "success" });
      navigate("/equipment");
    } catch (err) {
        ...
        <div className="space-y-4">
          <div className="sticky top-0 z-30 bg-ligth/10 backdrop-blur py-2 flex justify-between items-center border-b border-brown/10 mb-4">
            <h3 className="text-lg font-black text-dark flex items-center gap-2">
              <Wrench size={22} className="text-brown" /> معدات فاتورة الشراء
            </h3>
            <button
              type="button"
              onClick={addItem}
        ...
        .map((eq) => (
          ...
          >
            {eq.itemName}
                    ...
                    <div className="text-right">
                      <label className="text-xs font-black text-dark">ملاحظات على المعدة</label>
                      <input
                        type="text"
                        className="w-full p-2.5 mt-1 bg-ligth/20 border border-brown/10 rounded-xl outline-none font-bold text-dark focus:border-brown"
                        placeholder="اختياري"
                  ...
                  <div className="pt-2 border-t border-brown/5">
                    <div className="bg-ligth/20 p-3 rounded-xl flex justify-between items-center">
                      <span className="text-xs font-black text-dark/80">إجمالي المعدة</span>
        ...
        <div className="space-y-4">
          ...
          >
            {loading ? (
              ...
              <>
                <Save size={22} /> إنشاء فاتورة المعدات وربط الحسابات المالية
              </>
            )}
          </button>
        </div>
